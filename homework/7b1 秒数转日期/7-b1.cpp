...
struct tj_time* tj_time_convert(int input_time)
{
	...
	result.tj_year = 1970;
	result.tj_month = 1;
	result.tj_day = 1 + input_time / (24 * 3600);
	input_time -= ((result.tj_day - 1) * (24 * 3600));
	result.tj_hour = 8 + input_time / 3600;
	input_time -= (result.tj_hour - 8) * 3600;
	result.tj_minute = input_time / 60;
	input_time -= result.tj_minute * 60;
	result.tj_second = input_time;

	int mon[12] = { 31,28,31,30,31,30,31,31,30,31,30,31 }, leap;
	while (result.tj_hour >= 24) {
		result.tj_hour -= 24;
		result.tj_day++;
	}
	while (result.tj_day > mon[(result.tj_month) - 1]) {
		leap = result.tj_year % 4 == 0 && result.tj_year % 100 != 0 || result.tj_year % 400 == 0 ? mon[1] = 29 : mon[1] = 28;
		result.tj_day -= mon[(result.tj_month) - 1];
		result.tj_month++;
